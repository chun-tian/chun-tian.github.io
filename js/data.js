(function () {
   ...
   function syncToggles() {
    ...
    n.checked = enabled && supported && granted;
}
...
window._sendPartnerNotification = function(title, body) {
    try {
        ...
        // 页面处于前台且窗口聚焦时，不发系统通知（避免打扰）
        if (!document.hidden && document.hasFocus()) return;

        const avatarSrc = (document.querySelector('#partner-avatar img') || {}).src;

        const notif = new Notification(title || '传讯', {
            body: body || '对方发来了消息',
            icon: avatarSrc,
            badge: avatarSrc,
            tag: 'partner-msg-' + Date.now(),
            renotify: true,
            requireInteraction: true,   // 手机上通知不会自动消失
            silent: false,
            vibrate: [200, 100, 200]    // 安卓设备震动
        });
