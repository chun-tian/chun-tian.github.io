function setupEventListeners() {
    try {
        ...
        initComboMenu(); 
                try { initMorePanel(); } catch (e) { console.warn('initMorePanel:', e); }
...
window.updateBubblePreviewFn = updateBubblePreview;
            ...
            // ===== 红包封面设置 =====
            const rpCoverInput = document.getElementById('redpacket-cover-input');
            const rpCoverPreview = document.getElementById('redpacket-cover-preview');
            const rpCoverUploadBtn = document.getElementById('redpacket-cover-upload');
            const rpCoverResetBtn = document.getElementById('redpacket-cover-reset');

            function refreshRpCoverPreview() {
                if (!rpCoverPreview) return;
                const cover = window.redpacketFeature && window.redpacketFeature.getCover
                    ? window.redpacketFeature.getCover() : null;
                if (cover) {
                    rpCoverPreview.style.backgroundImage = 'url(' + cover + ')';
                    rpCoverPreview.innerHTML = '';
                } else {
                    rpCoverPreview.style.backgroundImage = '';
                    rpCoverPreview.innerHTML = '<span style="position:relative;z-index:1;">暂无自定义封面</span>';
                }
            }
            refreshRpCoverPreview();

            if (rpCoverUploadBtn && !rpCoverUploadBtn._bound) {
                rpCoverUploadBtn._bound = true;
                rpCoverUploadBtn.addEventListener('click', function () {
                    if (rpCoverInput) rpCoverInput.click();
                });
            }
            if (rpCoverInput && !rpCoverInput._bound) {
                rpCoverInput._bound = true;
                rpCoverInput.addEventListener('change', function (e) {
                    const file = e.target.files && e.target.files[0];
                    if (!file) return;
                    if (file.size > 2 * 1024 * 1024) {
                        showNotification('封面请小于 2MB', 'error');
                        return;
                    }
                    const reader = new FileReader();
                    reader.onload = function (ev) {
                        const data = ev.target.result;
                        if (window.redpacketFeature && window.redpacketFeature.setCover) {
                            window.redpacketFeature.setCover(data);
                            refreshRpCoverPreview();
                            showNotification('红包封面已更新 ✦', 'success');
                        }
                    };
                    reader.readAsDataURL(file);
                    e.target.value = '';
            ...
            if (rpCoverResetBtn && !rpCoverResetBtn._bound) {
                rpCoverResetBtn._bound = true;
                rpCoverResetBtn.addEventListener('click', function () {
                    if (window.redpacketFeature && window.redpacketFeature.setCover) {
                        window.redpacketFeature.setCover(null);
                        refreshRpCoverPreview();
                        showNotification('已恢复默认封面', 'success');
                    }
                });
            }
...
function initMorePanel() {
    if (window._morePanelBound) return;
    window._morePanelBound = true;

    function closePanel() {
        var p = document.getElementById('input-more-panel');
        if (p) p.classList.remove('open');
        var icon = document.querySelector('#more-btn i');
    ...
    document.addEventListener('click', function (e) {
        ...
        if (e.target.closest('#more-btn')) {
            e.stopPropagation();
            var panel = document.getElementById('input-more-panel');
            if (!panel) return;
            ...
            if (isOpen) {
                ...
                var icon = e.target.closest('#more-btn').querySelector('i');
        ...
        // 点面板里的按钮：执行完自动收起
        if (e.target.closest('#input-more-panel .more-item')) {
            // 让原生事件先执行完，稍后收起
        ...
        if (panel && panel.classList.contains('open')) {
            if (!e.target.closest('#input-more-panel') && !e.target.closest('#more-btn')) {
                closePanel();
            }
        }

        // 打开表情面板时，关闭更多面板
        if (e.target.closest('#combo-btn')) {
            closePanel();
        }
    });
    ...
    if (input) {
        ...
        function initCoreListeners() {
            ...
            // 红包按钮
            const rpBtn = document.getElementById('redpacket-btn');
            if (rpBtn && !rpBtn._bound) {
                rpBtn._bound = true;
                rpBtn.addEventListener('click', function () {
                    if (typeof window.openSendRedpacketDialog === 'function') {
                        window.openSendRedpacketDialog();
                    }
                });
            }
        }
