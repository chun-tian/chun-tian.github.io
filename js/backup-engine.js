...
(function (global) {
    ...
    function dataUrlToBinary(dataUrl) {
        ...
        // 更宽容的正则：header 里允许空字段；body 允许任意字符
        ...
        var mime = (header.split(';')[0] || '').trim() || 'application/octet-stream';
        ...
        if (isB64) {
            // 第一次尝试
            var bytes = tryAtob(body);
            if (bytes && bytes.length) return { mime: mime, bytes: bytes };

            // 兜底 1：补齐 base64 padding
            try {
                var padded = body;
                while (padded.length % 4 !== 0) padded += '=';
                bytes = tryAtob(padded);
                if (bytes && bytes.length) return { mime: mime, bytes: bytes };
            } catch (e1) {}

            // 兜底 2：URL-safe base64 (- _ 换成 + /)
            try {
                var urlSafe = body.replace(/-/g, '+').replace(/_/g, '/');
                while (urlSafe.length % 4 !== 0) urlSafe += '=';
                bytes = tryAtob(urlSafe);
                if (bytes && bytes.length) return { mime: mime, bytes: bytes };
            } catch (e2) {}

            // 全部失败 → 返回 null（会走 .txt 分支）
            console.warn('[backup] dataUrlToBinary 无法解析：', {
                headerLen: header.length,
                bodyLen: body.length,
                bodyHead: body.slice(0, 40),
                bodyTail: body.slice(-20)
            });
            return null;
        }
        ...
        try {
            ...
            // 兜底：至少当成原始字符串存
            try {
                return { mime: mime, bytes: new TextEncoder().encode(body) };
            } catch (e4) {
                return null;
            }
        }
    ...
    function inferBackupSessionId(lfKeys, appPrefix) {
        ...
        // 全局键（朋友圈 / 影音室 / 陪伴）不做 session id 重映射
        if (key.indexOf('socialMoments') === 0) return key;
        if (key.indexOf('mediaRoom_') === 0) return key;
        if (key.indexOf('companion_') === 0) return key;
    ...
    function buildModuleSkipPatterns(flags) {
        ...
        if (!flags.inclDg) p.push('dg_custom_data', 'dg_status_pool', 'weekly_fortune', 'daily_fortune', 'customWeather_');
                if (!flags.inclMoments) p.push('socialMoments_');
        if (!flags.inclMoments) p.push('socialMoments_v');
                if (!flags.inclRooms) p.push('mediaRoom_', 'companion_');
        return p;
    }
    ...
    async function buildBackupPayload(flags) {
        flags = flags || {
            ...
            inclMoments: true, inclRooms: true
        };
        var lfData = {};
    ...
    async function applyBackupToStorage(data, opt) {
        ...
        // ── 关键改动：不再 remap sessionId ──
        // 原逻辑会把备份里 A 的数据 remap 到当前 B，导致 sessionList
        // 和 chatMessages 的 key 不一致，reload 后读不到消息。
        // 新逻辑：原样写入，然后把备份会话补进 sessionList，并让 reload
        // 后自动切到该会话。
        ...
        for (var k in lsRaw) {
            ...
            try {
                ...
                console.warn('[backup] localStorage 恢复失败', k, e2);
            }
        }

        // 保证备份的会话在 sessionList 里
        if (backupSid) {
            try {
                var slKey = appPfx + 'sessionList';
                var sl = await localforage.getItem(slKey);
                if (!Array.isArray(sl)) sl = [];
                if (!sl.some(function (s) { return s && s.id === backupSid; })) {
                    sl.push({
                        id: backupSid,
                        name: (data.sessionName || '恢复的会话') + ' · ' + new Date().toLocaleDateString('zh-CN'),
                        createdAt: Date.now()
                    });
                    await localforage.setItem(slKey, sl);
                }
            } catch (e5) {
                console.warn('[backup] sessionList 更新失败', e5);
            }

            // 备份会话不是当前会话时，让 reload 后自动切过去
            if (backupSid !== curSid) {
                try {
                    localStorage.setItem('__PENDING_SESSION_SWITCH__', backupSid);
                } catch (e6) {}
                try {
                    await localforage.setItem(appPfx + 'lastSessionId', backupSid);
                } catch (e7) {}
            }
        }

        if (typeof APP_PREFIX !== 'undefined' && typeof SESSION_ID !== 'undefined') {
            if (!backupSid || backupSid === curSid) {
