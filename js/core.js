...
const loadData = async () => {
    try {
        ...
        const results = await Promise.allSettled([
            ...
            localforage.getItem(`${APP_PREFIX}globalReplies`)
        ]);
        ...
        const savedGlobalReplies = getVal(21);
...

        {
            ...
            const dbMsgs = (savedMessages && Array.isArray(savedMessages))
                ? savedMessages.map(m => ({ ...m, timestamp: new Date(m.timestamp) }))
                : [];

            const bkMsgs = (backup && Array.isArray(backup.messages) && backup.messages.length > 0)
                ? backup.messages.map(m => ({ ...m, timestamp: new Date(m.timestamp) }))
                : [];
            ...
            const dbTs = lastTs(dbMsgs);
            const bkTs = lastTs(bkMsgs);

            if (bkMsgs.length > 0 && (dbMsgs.length === 0 || bkTs > dbTs)) {
                // 备份比主存储更新（或主存储为空）→ 用备份
                console.warn('[loadData] 使用本地备份恢复：备份条数=' + bkMsgs.length + '，DB条数=' + dbMsgs.length);
                messages = bkMsgs;
                if (backup.settings) Object.assign(settings, backup.settings);
                if (backup.anniversaries && Array.isArray(backup.anniversaries)) {
                    anniversaries = backup.anniversaries;
                }
                setTimeout(() => { try { saveData(); } catch (e) {} }, 800);
                setTimeout(() => {
                    showNotification(
                        `已恢复 ${messages.length} 条消息${backup._truncated ? '（最近200条）' : ''}`,
                        'warning', 5000
                    );
                }, 1500);
            } else {
        ...
        if (Array.isArray(savedGlobalReplies)) globalReplies = savedGlobalReplies;
...
function _tryRecoverFromBackup() {
    try {
        ...
        // ★ 关键修复：只接受当前 session 的备份，防止切换到新角色时误恢复旧角色的聊天记录
        if (backup && backup.sessionId && typeof SESSION_ID !== 'undefined' && SESSION_ID) {
            if (backup.sessionId !== SESSION_ID) {
                console.warn('[backup] 备份属于其他会话，已忽略。备份:', backup.sessionId, '当前:', SESSION_ID);
                return null;
            }
        }
        return backup;
...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'globalReplies',          val: () => localforage.setItem(`${APP_PREFIX}globalReplies`, globalReplies) },
...
if (msg.replyTo) {
    ...
    messageHTML += `<div class="reply-indicator" data-reply-id="${escapeHtml(String(msg.replyTo.id || ''))}" style="cursor:pointer;" onclick="scrollToQuotedMessage(this)"><span class="reply-indicator-sender">${escapeHtml(repliedSender)}</span><span class="reply-indicator-text">${escapeHtml(repliedText)}</span></div>`;
}

    const isVoice = !!(msg.voice || msg.type === 'voice');
    const isRedpacket = !!(msg.redpacket || msg.type === 'redpacket');
    const isAskYou = !!(msg.askYouData);
    const isImageOnly = !msg.text && !isVoice && !isRedpacket && !isAskYou && !!msg.image;
    let content = '';
    if (isVoice) {
        content = (typeof window._renderVoiceBubble === 'function')
            ? window._renderVoiceBubble(msg)
            : '<div style="opacity:0.5;font-size:12px;">[语音]</div>';
    } else if (isRedpacket) {
        content = (typeof window._renderRedpacketBubble === 'function')
            ? window._renderRedpacketBubble(msg)
            : '<div style="opacity:0.5;font-size:12px;">[红包]</div>';
    } else if (isAskYou) {
        const d = msg.askYouData;
        const modeLabel = d.mode === 'single' ? '单选' : d.mode === 'multi' ? '多选' : '问答';
        const modeCls = d.mode === 'qa' ? 'qa' : d.mode;
        let optsHtml = '';
        if (d.mode !== 'qa' && Array.isArray(d.options)) {
            optsHtml = '<div class="ayb-opts">' +
                d.options.map(function (opt, i) {
                    const isChosen = d.answered && d.answer && d.answer.indexOf(opt) !== -1;
                    return '<div class="ayb-opt' + (isChosen ? ' ayb-opt-chosen' : '') + '">' +
                             '<span class="ayb-opt-letter">' + String.fromCharCode(65 + i) + '</span>' +
                             '<span class="ayb-opt-text">' + escapeHtml(opt) + '</span>' +
                           '</div>';
                }).join('') +
                '</div>';
        }
        content = '<div class="ayb-card">' +
                    '<div class="ayb-head">' +
                      '<span class="ayb-badge ayb-badge-' + modeCls + '">' + modeLabel + '</span>' +
                      '<span class="ayb-title">问问你</span>' +
                    '</div>' +
                    '<div class="ayb-question">' + escapeHtml(d.question) + '</div>' +
                    optsHtml +
                  '</div>';
    } else {
content = msg.text ? `<div>${escapeHtml(msg.text).replace(/\n/g, '<br>')}</div>` : '';
    ...
    if (!isVoice && !isRedpacket && !isImageOnly && msg.text && window.LinkCard) {
        try { content += window.LinkCard.buildCardsHtml(msg.text); } catch (e) {}
    }
    messageHTML += content;
    ...
    if (isImageOnly) {
        ...
        messageDiv.style.minWidth = '190px';
    } else {
        messageDiv.className = `message message-${msg.sender === 'user' ? 'sent' : 'received'} ${settings.bubbleStyle}`;
    }
    ...
    if (msg.sender === 'user' && settings.readReceiptsEnabled && isLastInSenderGroup) {
     ...
     if (metaHTML !== '') {
        ...
        if (isVoice && msg.voice && msg.voice.transcript) {
            const tr = document.createElement('div');
            tr.className = 'voice-transcript';
            tr.textContent = msg.voice.transcript;
            if (msg.sender !== 'user' && msg.sender !== null) {
                tr.style.display = 'none';
                tr.dataset.hiddenTranscript = '1';
            }
    ...
    // ★ 网页链接预览卡片
    if (msg.linkPreview && window.WebPreview) {
        try {
            var _wpHolder = document.createElement('div');
            _wpHolder.className = 'wp-card-container';
            _wpHolder.innerHTML = window.WebPreview.buildCardHtml(msg.linkPreview);
            if (_wpHolder.firstChild) {
                contentWrapper.appendChild(_wpHolder.firstChild);
            }
        } catch (e) {}
    }
    ...
    throttledSaveData();
        ...
        function sendMessage(textOverride = null, type = 'normal') {
            ...
            const createMessage = (imgSrc = null) => {
                ...
                // ★ 自动附加网页预览（输入框是纯 URL 且缓存里有预览数据）
                try {
                    if (window.WebPreview && text && window.WebPreview.isPureUrl(text)) {
                        const _wpUrl = window.WebPreview.extractUrl(text);
                        if (_wpUrl) {
                            const _wpCache = window.WebPreview.getCache();
                            if (_wpCache && _wpCache[_wpUrl]) {
                                messageData.linkPreview = _wpCache[_wpUrl];
                            }
                        }
                    }
                } catch (e) {}
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            if ((!customReplies || customReplies.length === 0) && (!globalReplies || globalReplies.length === 0)) {
            ...
            const allReplies = [...(customReplies || []), ...(globalReplies || [])];
            const replyPoolOnce = allReplies
...
const shouldSendSticker = _hubEmojiOn && enabledStickerPool.length > 0 && Math.random() < 0.2;

let finalText = replyText;
// ── 陪伴空间 Hub 统一设置（评论拼接 + 表情包开关）──
var _hubSettings = null;
try {
    var _hubRaw = localStorage.getItem('companionHubSettings_v1');
    if (_hubRaw) _hubSettings = JSON.parse(_hubRaw);
} catch(_e) {}
var _hubEmojiOn = !(_hubSettings && _hubSettings.emojiEnabled === false);
if (_hubSettings && _hubSettings.commentEnabled && Math.random() < (_hubSettings.commentChance || 0.15)) {
    var _pool = (customReplies || []).filter(function(x){ return x && typeof x === 'string' && x.length > 1 && x.length < 20; });
    if (_pool.length > 0) {
        var _c = _pool[Math.floor(Math.random() * _pool.length)];
        var _patterns = ['（' + _c + '）', '……' + _c, '（小声：' + _c + '）', _c + '~'];
        finalText = finalText + ' ' + _patterns[Math.floor(Math.random() * _patterns.length)];
    }
}
                    let separateEmoji = null;
if (_hubEmojiOn && customEmojis && customEmojis.length > 0 && Math.random() < 0.2) {
        ...
        finalText = Math.random() < 0.5
            ...
            : finalText + ' ' + emoji;
                    ...
                    var _voiceChance = (typeof settings !== 'undefined' && typeof settings.partnerVoiceChance === 'number')
                        ? settings.partnerVoiceChance
                        : 0.2;
                    var _useVoice = finalText && finalText.length >= 3 && Math.random() < _voiceChance;
                    var _voiceDur = Math.max(2, Math.min(60,
                        Math.round(finalText.length / 4) + 2 + Math.floor(Math.random() * 4)
                    ));

                    var _replyTo = (i === 0 && recentUserMsgs.length > 0 && Math.random() < 0.3)
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (_impConfirmBtn) _impConfirmBtn.onclick = function() {
                        ...
                        const _curName = (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '当前联系人';
                        if (doMsgs && messages.length > 0 && !confirm('导入将覆盖「' + _curName + '」的聊天记录，确定继续吗？')) return;
                        closeDialog();
...
window.initializeSession = async function() {
    ...
    // 【新增】优先检查角色切换标记
    // ------------------------------------------------------------
    // 角色切换时，personas.js 会写入：
    //   localStorage.setItem('__PENDING_SESSION_SWITCH__', 目标sessionId)
    // 然后 reload。这里第一件事就是读它，读到就直接用，不再走
    // hash 匹配 → lastSessionId → sessionList[0] 的 fallback 链，
    // 避免目标 session 不在 sessionList 时被回退到旧角色。
    // ============================================================
    var pendingSid = null;
    try {
        pendingSid = localStorage.getItem('__PENDING_SESSION_SWITCH__');
        if (pendingSid) localStorage.removeItem('__PENDING_SESSION_SWITCH__');
    } catch (e) {}

    if (pendingSid) {
        // 目标会话若不在列表里，补进去（修复孤立 persona）
        if (!sessionList.some(s => s.id === pendingSid)) {
            sessionList.push({
                id: pendingSid,
        ...
        // ========================================================
        // 【原有逻辑保持不变】正常启动：hash → lastSessionId → 第一个
        // ========================================================
...
window.initializeSession = async function() {
    ...
    // 让 URL 的 hash 与当前 SESSION_ID 保持一致
    try {
