...
async function switchToPersona(personaId) {
    ...
    // 2. ★ 关键修复：禁止任何后续操作再写紧急备份
    //    （避免 beforeunload / pagehide / visibilitychange 把当前角色的聊天记录写进
    //     localStorage，导致 reload 后被新角色误读）
    ...
    // 4. ★ 清空内存中的 messages，防止 reload 前有异步任务渲染旧角色的聊天记录
    try {
    ...
    // 6. 通过 hash 指定目标会话，然后 reload
    //    （reload 会销毁所有定时器与内存状态，保证切换干净）
    // ★ 写入"待切换"标记，reload 后由 initializeSession 优先读取
    try {
