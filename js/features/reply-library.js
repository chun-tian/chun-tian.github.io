...
let _repliesScope = 'personal';   // 'personal' | 'global'
let _searchQuery = '';
...
function _renderListContentOnly() {
    ...
    if (currentMajorTab === 'reply') {
        if (currentSubTab === 'custom') {
            itemsToRender = (_repliesScope === 'global') ? globalReplies : customReplies;
...
function _renderModernToolbar() {
    ...
    toolbar.innerHTML = `
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:10px 15px;border-bottom:1px solid var(--border-color);">
            ...
            <button class="toolbar-icon-btn ${_repliesScope === 'global' ? 'active' : ''}" id="tb-scope-toggle" title="${_repliesScope === 'global' ? '当前：通用字卡（所有角色共享）· 点击切换' : '当前：专属字卡（仅此角色）· 点击切换'}">
                <i class="fas ${_repliesScope === 'global' ? 'fa-globe' : 'fa-user'}"></i>
            </button>` : ''}
    ...
    const scopeToggle = toolbar.querySelector('#tb-scope-toggle');
    if (scopeToggle) {
        scopeToggle.onclick = function () {
            _repliesScope = (_repliesScope === 'global') ? 'personal' : 'global';
            _batchSelectedIndices.clear();
            _activeGroupFilter = null;
            if (typeof showNotification === 'function') {
                showNotification(_repliesScope === 'global'
                    ? '现在查看：通用字卡（所有角色共享）'
                    : '现在查看：专属字卡（仅此角色）', 'info', 1800);
            }
            renderReplyLibrary();
        };
    }
...
function _renderStickerTab(list, itemsToRender) {
    ...
    itemsToRender.forEach((item, index) => {
        ...
        div.innerHTML = `
            <img src="${item}" loading="lazy" onerror="this.style.opacity='0.25';this.style.filter='grayscale(1)';this.title='链接加载失败';">
...
function _batchToggleDisable() {
    const set = _getDisabledItemsSet();
    const srcArr = (_repliesScope === 'global') ? globalReplies : customReplies;
    const selectedItems = [..._batchSelectedIndices].map(i => srcArr[i]);
...
function _runDedup() {
    ...
    const grDedup = deduplicateContentArray(globalReplies);
    globalReplies = grDedup.result; totalRemoved += grDedup.removedCount;
...
function deleteItem(index) {
    ...
    if (currentMajorTab === 'reply' && currentSubTab === 'custom') {
        if (_repliesScope === 'global') globalReplies.splice(index, 1);
        else customReplies.splice(index, 1);
    }
...
function editItem(index, oldText) {
    ...
    if (currentMajorTab === 'reply' && currentSubTab === 'custom') {
        if (_repliesScope === 'global') globalReplies[index] = newText.trim();
        else customReplies[index] = newText.trim();
    }
...
function _showStickerAddMenu() {
    ...
    panel.innerHTML = `
        <style>@keyframes popIn{from{opacity:0;transform:scale(.93)}to{opacity:1;transform:scale(1)}}</style>
        <div style="font-size:16px;font-weight:700;color:var(--text-primary);margin-bottom:2px;">添加表情包</div>
        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;">选择导入方式</div>
        <button id="_sa_file" style="display:flex;align-items:center;gap:12px;padding:14px 16px;border:1.5px solid var(--border-color);border-radius:14px;background:var(--primary-bg);cursor:pointer;text-align:left;font-family:var(--font-family);transition:border-color 0.15s;">
            ...
            <div>
                <div style="font-size:13px;font-weight:600;color:var(--text-primary);">从本地文件</div>
                <div style="font-size:11px;color:var(--text-secondary);margin-top:2px;">支持 PNG / JPG / GIF 动图</div>
            </div>
        </button>
        <button id="_sa_link" style="display:flex;align-items:center;gap:12px;padding:14px 16px;border:1.5px solid var(--border-color);border-radius:14px;background:var(--primary-bg);cursor:pointer;text-align:left;font-family:var(--font-family);transition:border-color 0.15s;">
            <div style="width:38px;height:38px;border-radius:10px;background:rgba(var(--accent-color-rgb),0.12);display:flex;align-items:center;justify-content:center;color:var(--accent-color);flex-shrink:0;">
                <i class="fas fa-link" style="font-size:13px;"></i>
            </div>
            <div>
                <div style="font-size:13px;font-weight:600;color:var(--text-primary);">从链接批量导入</div>
                <div style="font-size:11px;color:var(--text-secondary);margin-top:2px;">每行一个 URL · 支持 GIF / WebP</div>
            </div>
        </button>
        <button id="_sa_cancel" style="padding:12px;border:1.5px solid var(--border-color);border-radius:13px;background:none;color:var(--text-secondary);font-size:13px;cursor:pointer;font-family:var(--font-family);margin-top:6px;">取消</button>
    `;
    overlay.appendChild(panel);
    document.body.appendChild(overlay);

    panel.querySelector('#_sa_cancel').onclick = () => overlay.remove();
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });

    panel.querySelector('#_sa_file').onclick = () => {
        overlay.remove();
        document.getElementById('sticker-file-input')?.click();
    };
    panel.querySelector('#_sa_link').onclick = () => {
        overlay.remove();
        _showStickerLinkImport();
    };
}

/* ---------- 表情链接批量导入 ---------- */
function _showStickerLinkImport() {
    const overlay = _makeOverlay();
    const panel = document.createElement('div');
    panel.style.cssText = `
        background:var(--secondary-bg);border-radius:22px;padding:22px;
    ...
    panel.innerHTML = `
        <style>@keyframes popIn{from{opacity:0;transform:scale(.93)}to{opacity:1;transform:scale(1)}}</style>
        <div style="font-size:16px;font-weight:700;color:var(--text-primary);margin-bottom:6px;">从链接批量导入表情</div>
        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:14px;line-height:1.6;">
            每行一个图片链接，支持 PNG / JPG / GIF / WebP。<br>
            <span style="color:var(--accent-color);">GIF 和 WebP 会保留动态效果。</span>
        </div>
        <textarea id="_sli_input" placeholder="https://example.com/cat.gif&#10;https://example.com/dog.png&#10;https://example.com/rabbit.webp" rows="8" style="
            width:100%;box-sizing:border-box;padding:12px 14px;
            border:1.5px solid var(--border-color);border-radius:13px;
            background:var(--primary-bg);color:var(--text-primary);
            font-size:12.5px;font-family:monospace;outline:none;resize:vertical;
            line-height:1.7;transition:border-color 0.18s;min-height:150px;
        "></textarea>
        <div id="_sli_hint" style="font-size:11px;color:var(--text-secondary);margin-top:8px;min-height:16px;"></div>
        <div style="display:flex;gap:10px;margin-top:14px;">
            <button id="_sli_cancel" style="flex:1;padding:12px;border:1.5px solid var(--border-color);border-radius:13px;background:none;color:var(--text-secondary);font-size:13px;cursor:pointer;font-family:var(--font-family);">取消</button>
            <button id="_sli_confirm" style="flex:2;padding:12px;border:none;border-radius:13px;background:var(--accent-color);color:#fff;font-size:14px;font-weight:700;cursor:pointer;font-family:var(--font-family);">导入</button>
        </div>
    `;
    overlay.appendChild(panel);
    document.body.appendChild(overlay);

    const ta = panel.querySelector('#_sli_input');
    const hint = panel.querySelector('#_sli_hint');
    ta.focus();

    ta.addEventListener('input', () => {
        const lines = ta.value.split('\n').map(l => l.trim()).filter(Boolean);
        if (lines.length > 0) {
            hint.textContent = `已识别 ${lines.length} 个链接`;
    ...
    panel.querySelector('#_sli_cancel').onclick = () => overlay.remove();
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });

    panel.querySelector('#_sli_confirm').onclick = () => {
        const lines = ta.value.split('\n').map(l => l.trim()).filter(Boolean);
        if (!lines.length) {
            hint.textContent = '请至少输入一个链接';
        ...
        const validUrls = [];
        const seen = new Set(stickerLibrary);
        let invalidCount = 0;
        let dupCount = 0;

        lines.forEach(url => {
            if (!urlPattern.test(url)) { invalidCount++; return; }
            if (seen.has(url)) { dupCount++; return; }
            seen.add(url);
            validUrls.push(url);
        });

        if (!validUrls.length) {
            hint.textContent = '没有可导入的有效链接' + (dupCount > 0 ? `（${dupCount} 个已存在）` : '');
            hint.style.color = '#ef4444';
            return;
        }

        validUrls.forEach(url => stickerLibrary.push(url));
        throttledSaveData();
        renderReplyLibrary();
        overlay.remove();

        const parts = [`✓ 已导入 ${validUrls.length} 个表情`];
        if (dupCount > 0) parts.push(`跳过 ${dupCount} 个重复`);
        if (invalidCount > 0) parts.push(`${invalidCount} 个格式不正确`);
        if (typeof showNotification === 'function') showNotification(parts.join('，'), 'success', 3000);
    };
}
...
function _showBatchAddDialog() {
    ...
    panel.querySelector('#ba-confirm').onclick = () => {
        ...
        lines.forEach(val => {
            ...
            if (currentSubTab === 'custom') {
                if (_repliesScope === 'global') { globalReplies.push(val); newItems.push(val); }
                else { customReplies.push(val); newItems.push(val); }
            }
