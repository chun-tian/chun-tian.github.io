...
function deduplicateContentArray(arr, baseSystemArray = []) {
    ...
    return { result, removedCount };
        ...
        const throttledSaveData = () => {
            // 1. 先同步把最新数据写进 localStorage 备份（不防抖，立刻执行）
...
function applyCustomBubbleCss(cssCode) {
    ...
    function boostSpecificity(css) {
        return css.replace(/([^{}@][^{}]*)\{([^{}]*)\}/g, (match, rawSel, body) => {
            ...
            // 跳过 @keyframes 里的帧标记（from / to / 0% / 50% 等）
            if (sel === 'from' || sel === 'to' || /^\d+(\.\d+)?%$/.test(sel)) return match;

            // 跳过含引号或 url() 的规则，避免破坏 data-URL 或字符串字面量
            if (/["']|\burl\s*\(/.test(sel) || /["']|\burl\s*\(/.test(body)) return match;

            // 拆分多选择器并逐个 boost
            const selectors = sel.split(',').map(s => s.trim()).filter(Boolean);
            const boosted = selectors.map(s => {
                if (!s) return s;
                if (s.startsWith('html') || s.startsWith('body')) return s;
                return `html body ${s}`;
            });
    ...
    try {
        ...
        const categories = [
            ...
            },
            {
                id: 'replies',
                label: '回复 / 拍一拍 / 氛围',
                indexedDBNeedles: ['customReplies', 'globalReplies', 'customPokes', 'customStatuses', 'customMottos', 'customIntros', 'customEmojis', 'customReplyGroups', 'customPokeGroups', 'customStatusGroups'],
            ...
            },
            {
                ...
                localStorageNeedles: []
                        ...
                        {
                id: 'moments',
                label: '朋友圈（动态 / 封面 / 点赞评论）',
                indexedDBNeedles: ['socialMoments'],
                localStorageNeedles: []
            },
                        {
                id: 'rooms',
                label: '影音室 / 陪伴空间（消息 / 背景 / 清单 / 倒计时）',
                indexedDBNeedles: ['mediaRoom_', 'companion_'],
                localStorageNeedles: []
            },
            {
                id: 'themes',
                label: '主题 / 外观 / 图库 / 红包封面',
                            indexedDBNeedles: ['customThemes', 'themeSchemes', 'backgroundGallery', 'chatBackground', 'partnerAvatar', 'myAvatar', 'partnerPersonas', 'redpacketCover', 'personaList'],
                localStorageNeedles: []
            },
            {
                id: 'dg',
...
/**
 * HTML 转义：防止用户输入被当作 HTML 标签执行
 * 所有把用户文本拼进 innerHTML 的地方，都必须先调用它
 */
